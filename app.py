...
def update_repair(row_id, job_no, name, phone, brand, model, issue, parts, status, repair_date):
    ...
    c.execute('''
        UPDATE repairs 
        SET job_no=?, customer_name=?, phone_number=?, brand=?, model=?, issue=?, parts_used=?, status=?, repair_date=?
        WHERE id=?
    ''', (str(job_no).strip(), str(name).strip(), str(phone).strip(), str(brand).strip(), str(model).strip(), str(issue).strip(), str(parts).strip(), str(status).strip(), str(repair_date).strip(), int(row_id)))
    conn.commit()
    conn.close()
...
def delete_repair(row_id):
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("DELETE FROM repairs WHERE id=?", (int(row_id),))
    conn.commit()
    conn.close()

def get_all_repairs():
    conn = sqlite3.connect(DB_PATH)
    # 📌 บังคับ ORDER BY id DESC เพื่อดึงรายการล่าสุดขึ้นก่อนเสมอ
...
def is_job_duplicate_on_update(job_no, current_id):
    ...
    conn = sqlite3.connect(DB_PATH)
    c = conn.cursor()
    c.execute("SELECT 1 FROM repairs WHERE job_no = ? AND id != ? LIMIT 1", (str(job_no).strip(), int(current_id)))
    result = c.fetchone()
    conn.close()
    return result is not None
...
# =======================================================
# 🔍 1. เมนูค้นหาและจัดการข้อมูลใบงาน (แสดงข้อมูลล่าสุดเสมอ)
# =======================================================
if st.session_state.active_menu == "จัดการใบงาน":
    ...
    with st.container():
        ...
        if not df_repairs.empty:
            ...
            # 📌 เรียงลำดับเอาข้อมูลล่าสุด (id หรือ date_added) ขึ้นบนสุดเสมอ
